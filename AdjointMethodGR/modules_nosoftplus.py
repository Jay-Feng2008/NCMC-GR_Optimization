...
class BrillLindquistMetric(nn.Module):
    def __init__(self, num_sources, init_masses=None, init_positions=None):
        super().__init__()

        if init_masses is not None:
            masses = torch.tensor(init_masses, dtype=torch.float32)
        else:
            masses_ = torch.abs(torch.rand(num_sources, dtype=torch.float32))

        self.masses = nn.Parameter(init_masses) 
    ...
    def forward(self, x):
        ...
        points_spatial = torch.stack((x1, x2, x3), dim=-1)  # (..., 3)
...
class BrillLindquistChristfSym(nn.Module):
    ...
    def forward(self, x):
        ...
        for b in range(B):
            position = x[b]  # (n,)
            G = self.metric_field(position)  # (n, n)
...
class NeuralODEfunc(nn.Module):
    def __init__(self, positions, masses):
        super().__init__()
        self.metric_field = BrillLindquistMetric(num_sources=len(masses), 
                                                 init_masses=masses, init_positions=positions)

...
class NeuralODEfunc(nn.Module):
    ...
    def update_funcs(self, chi, gamma_bar_mat_lo_raw, A_bar_mat_lo_raw, Gamma_bar, K):
        ...
        gamma_bar_mat_lo = torch.einsum('xyz,xyzij->xyzij',torch.linalg.det(gamma_bar_mat_lo_raw)**(-1/3),gamma_bar_mat_lo_raw)     # Actually an evolving varible; enforce constraints
        ...
        # ↑ Actually an evolving varible; enforce constraints
    ...
    def forward(self, t, x):
        ...
        # Gamma^i_{jk} V^k -> (B,i,j)
        GammaV = torch.einsum('bijk,bk->bij', Gamma_b, V)
        term1 = grad_ln_alpha - KdotV  # (B,j)
        Vi_term = V.unsqueeze(2) * term1.unsqueeze(1)  # (B, i, j) 
        bracket = Vi_term + 2.0 * K_mixed - GammaV  
